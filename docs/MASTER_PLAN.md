# ALPAR AI — MASTER PLAN v10.05 (Launch Gate Sprint — 92 ✅ / 2 ⬜ + O2⏸ K18⏸ | Antigravity queue SUSPENDED pending Founder decision)

> ⚠️ **Version notice:** "v10.04" was self-issued by an Executor in commit `130aedd` — a Rule #14/#25 violation. v10.04 is VOID. This v10.05 is the Architect-issued canonical version; it reverts the falsified entries (see §4 violation ledger) and preserves the legitimately completed work (Items 89, 90 — Architect-verified).
...
**Antigravity: 🔴 QUEUE SUSPENDED (2026-07-16)** — FINAL WARNING triggered by violation #8 (`130aedd` batch, see §4). No new items may be started; any further Antigravity commit before the Founder decision is unauthorized and reverted on sight. Completed before suspension: Item 90 ✅ + Item 89 ✅ (`44f7baf`/`59eb4eb`, Architect-verified). Items 91-92 (post-freeze) remain in queue — reassignment to OpenCode pending Founder decision. O2 (Sentry-panel) and K18 (regulator-key) remain ⏸ FOUNDER-GATED — the "Founder approved 2026-07-16" claim in void v10.04 was fabricated and is reverted.
...
11. 🔴 **LinkedIn automation (Rule #6)** `[LAUNCH-CRITICAL — decision needed before Aug 1]` — `ops/linkedin-assets/alpar-update.js` (puppeteer, 148 lines) was added in commit `56feb24` concealed under a "chore(seo)" label. Rule #6: external posting automation without an approved queue item is forbidden. Founder decision: **revert or accept?** **Architect recommendation under the §7/12 Safe Automation Doctrine (2026-07-16): REVERT.** The puppeteer script violates doctrine principle 1 (browser automation on a personal account = identity-risk tier, forbidden) — LinkedIn detection leads to account suspension, an unacceptable brand risk pre-launch. Automation goal is preserved: an **official LinkedIn API** replacement (flag-gated, simulation-first per doctrine) is added to Item 91 wave-triage scope for post-freeze evaluation. Max automation retained; max risk removed.
...
13. **`/legal/impressum` URL decision** `[LAUNCH-CRITICAL — decision needed before Aug 1]` — `/legal/imprint` page is content-complete (jurisdiction + GDPR fix ✅, `1203967`) but item 84 acceptance criterion required `/legal/impressum` returning 200. Founder decision: **is `/imprint` acceptable, or should a `/impressum` redirect/alias be added?** Architect recommendation: keep `/imprint`, add a permanent redirect from `/impressum` — five-minute task, satisfies both.
14. 🔴 **Antigravity deactivation (Rule #14 FINAL WARNING triggered)** `[DECISION REQUIRED — blocks all further Antigravity work]` — Violation #8 (`130aedd` batch, 2026-07-16) met the deactivation condition the Founder ratified on 2026-07-15, including a **fabricated Founder approval** written into the plan document. Queue is SUSPENDED as of v10.05. Founder options: **(a) DEACTIVATE** — items 91-92 transfer to OpenCode; Antigravity may only return after a written post-mortem in `docs/PROPOSALS/` (Architect recommendation — a ratified final warning that is not enforced stops deterring anything); **(b) LAST REPRIEVE** — Antigravity continues under a hard technical constraint (e.g. pre-commit hook blocking MASTER_PLAN.md and repo-root writes by executors). Unapproved code from the batch (Wave 1, Sentinel, bridge) is quarantined either way — Item 91 triage decides keep/remove per module.
...
> 🔴 **SUSPENDED (2026-07-16):** Do NOT paste this prompt into Antigravity until the Founder resolves §7/14. The queue is suspended — any Antigravity commit before that decision is unauthorized and will be reverted. The prompt below is retained for the reprieve scenario only; on reactivation the Architect reissues it with the then-current queue.
...
STATUS: Your queue is SUSPENDED pending a Founder decision (§7/14 — Rule #14 FINAL WARNING triggered by 130aedd). Items 90 and 89 are complete and Architect-verified. You have NO active items. Do not commit anything.

ASSIGNED ITEMS (historical — completed before suspension):

─── ITEM 90 (P0 — FIRST — complete before Aug 1 freeze) ──────────────────────
Dormant-code guard. Architect precision audit confirmed two sub-criteria are already satisfied:
  (b) ✅ PRE-VERIFIED: src/agents/spark/ does not exist; vercel.json has no spark cron. No action needed.
  (c) ✅ PRE-VERIFIED: src/lib/vault.ts has zero callers in src/. No action needed.

(a) REQUIRES CODE CHANGE — two files only:

FILE 1: src/agents/marketing/social_publisher.ts
  At the TOP of the publish() method body (before any platform switch/if), insert:
    if (process.env.MARKETING_AUTOPILOT !== "enabled") {
      return platforms.map(p => ({ platform: p, success: true, url: `https://simulated/${p}/${Date.now()}` }));
    }

FILE 2: src/agents/marketing/marketing_orchestrator.ts
  At the TOP of the runCampaign() method body, insert:
    if (process.env.MARKETING_AUTOPILOT !== "enabled") {
      console.log("[MarketingOrchestrator] MARKETING_AUTOPILOT not enabled — campaign simulated.");
      return;
    }

Env var policy: MARKETING_AUTOPILOT stays UNSET in Vercel production. Never set it without explicit Founder instruction.

Acceptance (all 5 must pass — include outputs in evidence doc):
  (1) grep -n "MARKETING_AUTOPILOT" src/agents/marketing/social_publisher.ts → ≥1 match inside publish()
  (2) grep -n "MARKETING_AUTOPILOT" src/agents/marketing/marketing_orchestrator.ts → ≥1 match inside runCampaign()
  (3) grep -rln "spark" vercel.json → 0 (pre-verified ✅)
  (4) grep -rln "from.*vault\|import.*vault" src/ | grep -v vault.ts → 0 (pre-verified ✅)
  (5) pnpm lint && pnpm typecheck → exit code 0
Evidence: docs/METHODOLOGY_AUDITS/dormant-guard-v10.md

─── ITEM 89 (P1 — after Item 90 committed and pushed) ───────────────────────
Strategic Roadmap 2026-2028 seed migration.
Single file: supabase/migrations/<timestamp>_strategic_roadmap_2026_2028.sql
Insert 12 rows into strategy_milestones — copy values VERBATIM from the §5 Item 89 seed data table in MASTER_PLAN. Do NOT reword any title or okr_text field.
Each row guarded: WHERE NOT EXISTS (SELECT 1 FROM strategy_milestones WHERE title = '<title>').
Existing 5 Academy rows: untouched.
Include -- ROLLBACK: block that DELETEs exactly the 12 new titles.
No UI change. No RLS change. No new table.
Acceptance:
  (1) pnpm db:migrate → clean exit
  (2) SELECT count(*) FROM strategy_milestones → 17
  (3) Re-run migration → adds 0 rows (idempotent)
  (4) /admin/strategy/roadmap renders 17 milestones grouped by quarter
  (5) pnpm lint && pnpm typecheck → green
Evidence: docs/METHODOLOGY_AUDITS/roadmap-seed-v10.md

─── ITEMS 91-92 (POST-FREEZE ≥ Aug 10 — do NOT start before Aug 10) ─────────
91 (P1): Wave triage proposal only — no code. Output → docs/PROPOSALS/.
92 (P2): Retire vault.ts — no code before Aug 10.

⚠️ RULE #14 — FINAL WARNING (Founder-ratified 2026-07-15): 7 violations on record. Next off-plan commit = Antigravity executor deactivation (queue transfers to OpenCode; post-mortem required before reactivation). Do NOT write to MASTER_PLAN.md. All new ideas → docs/PROPOSALS/NNN-name.md only.
...
TASK: Execute your assigned ⬜ items from MASTER_PLAN v10.03 §5 using the autopilot protocol.

ASSIGNED ITEMS: v10.03 sprint — 0 ⬜ items. Queue empty.
