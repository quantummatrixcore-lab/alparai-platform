...
| 36  | P0       | [Antigravity/OpenCode] Birim Test Paketi Onarımı — Fix Unit Test Suite | `a98b392`: 153/153 test dosyası (%100) ve 933/933 test (%100) PASSED. /admin/api-keys & /admin/providers rotaları eklendi, openrouter-gateway mock hoisting ve fetch-external zaman aşımı çözüldü. | ✅ completed |
| 37  | P0       | [Antigravity/OpenCode] Gece Otonom Güvenlik Taraması — Security Cron & Audit Fix | `45299e1`: `package.json` overrides seviyesinde `brace-expansion`, `minimatch` ve bağımlılık kilitleri eklendi, `pnpm install` ile kilit dosyası güncellendi. | ✅ completed |
| 38  | P1       | [Antigravity/OpenCode] Otonom Görev Kuyruğu Altyapısı — `automation_tasks` Migration | `45299e1`: `20260822000000_automation_tasks.sql` migration dosyası RLS politikaları, status indeksleri ve `-- ROLLBACK:` bloğu ile oluşturuldu. | ✅ completed |
| 39  | P1       | [Antigravity/OpenCode] Görsel Regresyon Kilit Sistemi — Playwright VRT Baselines | `6d0a9e1`: `playwright.config.ts` içine Doktrin #035 & #036 uyarınca %5 maks eşik değerli VRT kilidi (`maxDiffPixelRatio: 0.05`) eklendi. | ✅ completed |
| 40  | P1       | [Antigravity/OpenCode] 360° Google Ultra Ekosistem Entegrasyonu — Veo & Imagen 3 | Veo 2.0 & Imagen 3 medya üreticileri ve `src/lib/ai/adapters/` adaptör hattı doğrulandı, unit testleri green. | ✅ completed |
| 41  | P0       | [Antigravity/OpenCode] OpenCode Free & Nvidia Model Havuzu Yönlendirmesi | `4c499f6`: `OPENCODE_FREE_MODELS` ve `NVIDIA_NIM_MODELS` havuzları `openrouter-gateway.ts` ve `model-router.ts` içerisine entegre edildi (Doktrin #044). | ✅ completed |
