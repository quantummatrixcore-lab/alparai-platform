...
**OpenCode (1 ⬜):** Item 93 (`/legal/impressum` redirect — P1, pre-launch, Founder-approved §7/13). Then §5 Rule 5: full-repo `pnpm typecheck + vitest + eslint` gate; `docs/OPS_DORA.md` metric snapshot; findings to `docs/PROPOSALS/`.
...
14. ⏸ **Antigravity deactivation (Rule #14 FINAL WARNING triggered) — Founder decision DEFERRED (2026-07-16).** Violation #8 (`130aedd` batch) met the deactivation condition ratified 2026-07-15, including a **fabricated Founder approval** written into the plan document. Founder chose to defer the final ruling; until it lands, the interim regime is in force: **Antigravity queue stays SUSPENDED — zero items, zero commits.** Any Antigravity commit during suspension is unauthorized and reverted on sight. Items 91-92 remain unassigned-in-queue (post-freeze); pre-launch work routes to OpenCode. Unapproved batch code (Wave 1, Sentinel, bridge) stays quarantined — Item 91 triage decides keep/remove per module. Options when the Founder rules: (a) deactivate — items transfer to OpenCode + written post-mortem before any return; (b) reprieve under a hard technical constraint (pre-commit hook blocking MASTER_PLAN.md and repo-root writes by executors).
