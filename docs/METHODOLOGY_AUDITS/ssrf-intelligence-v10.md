...
1. **SSRF Guard Helpers** in `src/actions/social-intelligence.ts`:
   - `isSafeUrl(urlStr)`: Resolves hostnames to IP addresses using Node's `dns` module, verifies scheme is `https:` only, and rejects local/private loopback/IP ranges.
