...
test.describe("Accessibility", () => {
  test("skip navigation link is functional", async ({ page }) => {
    await page.goto("/en");
  ...
  test("all images have alt text or are decorative", async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(
        hasAccessibleLabel,
        `Image ${i} missing alt text or aria-hidden`
      ).toBeTruthy();
    }
  });
  ...
  test("language switcher works with keyboard", async ({ page }) => {
    await page.goto("/en");
  ...
  test("focusable elements have visible focus indicators", async ({
    page,
  }) => {
    await page.goto("/en");
    await page.keyboard.press("Tab");
