...
async function exec(fn: () => Promise<void>) {
  try {
    ...
    logger.error("[EcosystemAction] Action failed", {
...
export async function archiveEcosystemNews(id: string) {
  await exec(async () => {
    const supabase = createAdminClient();
...
export async function updateEcosystemCategory(id: string, category: string) {
  await exec(async () => {
    const supabase = createAdminClient();
