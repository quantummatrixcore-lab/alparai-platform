...
import { HelpCircle, ExternalLink, CheckCircle2, AlertTriangle, AlertCircle, Database, Server, RefreshCw } from "lucide-react";
...
export function ResourcesClient({ locale }: { locale: string }) {
  ...
  return (
    <div className="space-y-8 p-4 lg:p-8">
      ...
      <div
        className={`border-l-4 p-4 rounded-r-xl backdrop-blur-xl ${
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold">
              ...
              {systemStatus === "operational"
                ? isTurkish ? "Tüm Servisler Normal" : "Operational"
                : systemStatus === "warning"
                  ? isTurkish ? "Kapasite Uyarısı" : "Capacity Warning"
                  : isTurkish ? "Kritik Aşım" : "Degraded Performance"}
            </h3>
      ...
      <div className="bg-bg-secondary border-border-subtle rounded-xl border p-6">
        <h2 className="text-white text-lg font-bold tracking-tight mb-6 flex items-center gap-2">
          <Database className="h-5 w-5 text-brand-400" />
          {isTurkish ? "Canlı Kapasite Göstergeleri" : "Live Capacity Telemetry"}
        </h2>

        {loading ? (
          <div className="py-8 text-center text-fg-muted font-mono text-sm animate-pulse">
            {isTurkish ? "Canlı kapasite verileri sorgulanıyor..." : "Fetching live capacity metrics..."}
          </div>
        ) : !metrics ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {/* Supabase DB */}
            <div className="bg-bg-tertiary/20 p-5 rounded-xl border border-white/5 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start">
                  <span className="text-white font-bold text-sm">Supabase Database size</span>
                  <Badge variant="outline" className={getUsageColorClass(metrics.supabaseDb.percentage)}>
                    {metrics.supabaseDb.percentage.toFixed(1)}%
                  </Badge>
                </div>
                <p className="text-xs text-fg-muted mt-1 font-mono">
                  {(metrics.supabaseDb.usedBytes / 1024 / 1024).toFixed(2)} MB / 500 MB limit
                </p>
              </div>
            ...
            <div className="bg-bg-tertiary/20 p-5 rounded-xl border border-white/5 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start">
                  <span className="text-white font-bold text-sm">Supabase Storage</span>
                  <Badge variant="outline" className={getUsageColorClass(metrics.supabaseStorage.percentage)}>
                    {metrics.supabaseStorage.percentage.toFixed(1)}%
                  </Badge>
                </div>
                <p className="text-xs text-fg-muted mt-1 font-mono">
                  {(metrics.supabaseStorage.usedBytes / 1024 / 1024).toFixed(2)} MB / 1,024 MB limit
                </p>
              </div>
            ...
            <div className="bg-bg-tertiary/20 p-5 rounded-xl border border-white/5 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start">
                  <span className="text-white font-bold text-sm">Upstash Redis Commands</span>
                  <Badge variant="outline" className={getUsageColorClass(metrics.upstashRedis.percentage)}>
              ...
              <span className="text-[10px] text-fg-muted font-mono mt-2 block text-right">
            ...
            <div className="bg-bg-tertiary/20 p-5 rounded-xl border border-white/5 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start">
                  <span className="text-white font-bold text-sm">Resend Emails Sent</span>
                  <Badge variant="outline" className={getUsageColorClass(metrics.resendEmails.percentage)}>
            ...
            {/* AI Gateway Spend */}
            <div className="bg-bg-tertiary/20 p-5 rounded-xl border border-white/5 flex flex-col justify-between lg:col-span-2">
              <div>
                <div className="flex justify-between items-start">
                  <span className="text-white font-bold text-sm">AI Gateway Cost ceiling (Rule #20)</span>
                  <div className="flex gap-2">
                    <Badge variant="outline" className={getUsageColorClass((metrics.aiGateway.dailyUsed / metrics.aiGateway.dailyLimit) * 100)}>
                      Daily: {((metrics.aiGateway.dailyUsed / metrics.aiGateway.dailyLimit) * 100).toFixed(1)}%
                    </Badge>
                    <Badge variant="outline" className={getUsageColorClass((metrics.aiGateway.monthlyUsed / metrics.aiGateway.monthlyLimit) * 100)}>
                      Monthly: {((metrics.aiGateway.monthlyUsed / metrics.aiGateway.monthlyLimit) * 100).toFixed(1)}%
                    </Badge>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div>
                    <span className="text-xs text-fg-muted block">Today Spend</span>
                    <span className="text-white font-mono font-bold text-lg">${metrics.aiGateway.dailyUsed.toFixed(4)} / $10.00 cap</span>
                  </div>
                  <div>
                    <span className="text-xs text-fg-muted block">This Month Spend</span>
                    <span className="text-white font-mono font-bold text-lg">${metrics.aiGateway.monthlyUsed.toFixed(4)} / $30.00 cap</span>
                  </div>
                </div>
              </div>
      ...
      {metrics && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div className="bg-bg-secondary border-border-subtle rounded-xl border p-5">
            <span className="text-xs text-fg-muted uppercase font-bold tracking-wide">Incidents Row Count</span>
            <p className="text-white text-2xl font-black font-mono mt-2">{metrics.rowCounts.incidents.toLocaleString()}</p>
          </div>
          <div className="bg-bg-secondary border-border-subtle rounded-xl border p-5">
            <span className="text-xs text-fg-muted uppercase font-bold tracking-wide">K-Model Scores Count</span>
            <p className="text-white text-2xl font-black font-mono mt-2">{metrics.rowCounts.kModelScores.toLocaleString()}</p>
          </div>
          <div className="bg-bg-secondary border-border-subtle rounded-xl border p-5">
            <span className="text-xs text-fg-muted uppercase font-bold tracking-wide">External Queue size</span>
            <p className="text-white text-2xl font-black font-mono mt-2">{metrics.rowCounts.externalIncidentsQueue.toLocaleString()}</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-bg-secondary border-border-subtle rounded-xl border overflow-hidden">
        <div className="px-6 py-4 border-b border-border-subtle">
          <h2 className="text-white text-lg font-bold tracking-tight">
            {isTurkish ? "15 Altyapı ve AI Sağlayıcı Kataloğu" : "15 Infrastructure & AI Vendors Catalog"}
          </h2>
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="bg-bg-tertiary/50 border-b border-border-subtle text-fg-muted text-xs font-bold uppercase tracking-wider">
                <th className="px-6 py-4">{isTurkish ? "Sağlayıcı / Kategori" : "Vendor / Category"}</th>
                ...
                <th className="px-6 py-4">{isTurkish ? "Alternatifler & Geçiş Maliyeti" : "Alternatives & Switch Costs"}</th>
              </tr>
            </thead>
                ...
                <tr key={v.id} className="hover:bg-bg-tertiary/20 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-bold text-white flex items-center gap-1.5">
                      {v.name}
                      <a href={v.url} target="_blank" rel="noopener noreferrer" className="text-brand-400 hover:text-brand-300">
                        <ExternalLink className="h-3.5 w-3.5" />
                      </a>
                    </div>
                    <span className="text-xs text-fg-muted mt-0.5 block">{v.category}</span>
                  </td>
                  {/* Plan */}
                  <td className="px-6 py-4 text-fg-primary text-xs font-mono">
                    <Badge variant="outline" className="border-white/10 text-white bg-white/5">
                      {v.plan}
                    </Badge>
                  </td>
                  {/* Cost */}
                  <td className="px-6 py-4 text-fg-primary font-mono text-sm font-bold">
                    {v.cost}
                  </td>
                  {/* Pros & Cons */}
                  <td className="px-6 py-4 text-xs text-fg-muted max-w-sm">
