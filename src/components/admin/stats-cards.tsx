      ...
      {cards.map((c) => (
        <Card key={c.label}>
          <CardContent className="p-4">
            <div
              className={`bg-bg-tertiary mb-2 inline-flex h-8 w-8 items-center justify-center rounded-md ${c.color}`}
            >
              {c.icon}
            </div>
            <p className="text-fg-primary text-2xl font-bold">{c.value.toLocaleString()}</p>
