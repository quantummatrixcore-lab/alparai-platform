-- Migration: Strategy & RBAC Expansion
-- Timestamp: 2026-06-24 10:00:00

-- End transaction to allow enum alteration
COMMIT;

-- 1. Alter type user_role to add 'advisor' if it doesn't exist
-- Note: 'add value' cannot be run inside a transaction in standard postgres.
-- Using commit/begin wrapper allows this to run in Supabase migrations.
ALTER TYPE public.user_role ADD VALUE IF NOT EXISTS 'advisor';

-- Restart transaction
BEGIN;

-- 2. Add community_role and interests fields to users table
...
-- 3. Helpers to check CEO and Advisor status
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.users
    WHERE id = uid and role = 'advisor'
  );
$$;

-- 4. Create strategy_swot_items table
...
-- 6. Create strategy_valuations table
...
-- 7. Create strategy_milestones table
...
-- 8. Create strategy_metrics_snapshots table
...
-- 9. Enable RLS on all strategy tables
...
-- 10. Define RLS Policies for SWOT
...
-- 11. Define RLS Policies for Risks
...
-- 12. Define RLS Policies for Valuations
...
-- 13. Define RLS Policies for Milestones
...
-- 14. Define RLS Policies for Metrics Snapshots
