...
9. **Model routing (token economy)** — delegate discovery to Haiku; never scan the codebase directly with an expensive model:
   ...
   **TOM (Token Optimization Engine) — MASTER_PLAN doctrine entries:** Haiku drafts → Sonnet writes the full content → Opus reviews only (approve or diff-sized patch on architecture/governance/security grounds). Full rewrites at the Opus stage are prohibited — see G-4 in MASTER_PLAN v11.13.
